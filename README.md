...
- **Problem**: 400개 이상의 실시간 시세 행을 3초 주기로 전체 갱신 시, 불필요한 DOM 조작으로 인한 브라우저 메인 스레드 점유 및 스크롤 버벅임 발생.
- **Solution**: `IntersectionObserver` API를 활용하여 사용자 화면(Viewport)에 노출된 행만 업데이트하는 부분 렌더링 로직 구현.
- **Result**: 불필요한 리렌더링을 획기적으로 줄여 저사양 기기에서도 끊김 없는 실시간 테이블 구현.

### 2. 외부 API 의존성 극복 및 가용성 확보 (SWR & Multi-level Caching)
- **Problem**: 외부 거래소 API의 호출 제한(Rate Limit) 및 일시적 장애 시 서비스 전체가 중단되는 리스크 존재.
- **Solution**: 
...
- **Result**: API 호출 횟수 60% 이상 절감 및 외부 장애 시에도 이전 데이터를 표시하는 Fallback 구조로 서비스 안정성 확보.

### 3. 운영 환경 예외 처리 및 하위 호환성 유지 (Storage Versioning)
- **Problem**: 배포 후 데이터 구조 변경 시, 기존 유저의 로컬 스토리지에 남은 구버전 데이터와 충돌하여 화이트 스크린(Runtime Error) 발생.
- **Solution**: 앱 버전 키(`appVersion`)를 도입하여 버전 불일치 시 스토리지 자동 초기화 로직 구현 및 모든 스토리지 접근부 `try-catch` 방어 코드 적용.
- **Result**: 업데이트 후 발생하는 사용자 이탈 리스크 제거 및 다양한 브라우저 환경에서의 런타임 안정성 극대화.
