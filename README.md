...
#### 11. 초기 진입 로딩 지연 문제 (6초 → 2.5초 단축)

- **문제**: 초기 진입 시 다수의 외부 API 동시 호출로 3~6초 지연 발생 <br>특히 Upbit는 청크 호출 중 일부 데이터가 먼저 반영되며<br>
거래대금 정렬이 일시적으로 불안정하게 보이는 현상 발생.

- **해결**: 초기 마켓 범위 제한, Upbit KRW 마켓을 60개 단위로 분할 호출하되 <br>
전체 청크 수집 완료 후 단일 렌더 방식으로 변경하고 실패 청크 재시도 로직 추가.

- **결과**: 초기 체감 속도 약 6초 → 약 2.5초로 개선. <br>
부분 데이터로 인한 정렬 불안정 현상을 제거하고 첫 화면 안정성을 확보함.
