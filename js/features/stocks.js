(() => {
  ...
  function showTableSpinner() {
    if (el.spinner) el.spinner.style.display = "flex";
  ...
  async function refresh(forceFullRender = false) {
    ...
    const shouldShowSpinner = (forceFullRender && state.rows.length === 0) || (state.rows.length === 0 && !readCache());
    if (shouldShowSpinner) showTableSpinner();
    ...
    try {
      ...
      const needFull = forceFullRender || domRef.size === 0;
      render(needFull);
